...
def check_for_correctness(input_path: str, output_path: str, synthetic_dataset: bool = False):
    ...
    data = pd.read_json(input_path, lines=True)
        ...
        # Ensure output columns always exist, even for tiny/partial runs
    default_false_cols = [
        "correct_format",
    ...
    for idx, example in data.iterrows():
        ...
        if example['completion_string'] is None:
            print(f"WARN: For sample {idx} no completion is available. Consider manual recalculation.")
            continue

        prompt, completion_gt, _ = get_last_graph(example, synthetic_dataset)

        if example['completion_string'] is None:
            print(f"WARN: For sample {idx} no completion is available.")
            continue

        # Ground truth must start with 'e'
        ...
        try:
            ...
            print(f"WARN: sample {idx} ground truth could not be parsed: {e}")
            ...
            data.loc[idx, "correct_values"] = False
            continue

        # Generated completion must also start with 'e '
        completion_string = str(example['completion_string']).strip()
        ...
        try:
            graph_generated, _ = get_anchored_completion_graph(prompt, generated_completion, synthetic_dataset, version=V_JSON
        )
        except Exception as e:
            print(f"WARN: sample {idx} generated completion could not be parsed: {e}")
            graph_generated = None

        if graph_generated is None:
            data.loc[idx, "correct_format"] = False
            data.loc[idx, "type_isomorphic_completion"] = False
            data.loc[idx, "type_subgraph_isomorphic_generated_in_gt"] = False
            data.loc[idx, "type_subgraph_isomorphic_gt_in_generated"] = False
        ...
        if (synthetic_dataset):
            ...
            if (count_edges >= 0):
                ...
                data.loc[idx, "correct_format"] = (graph_generated is not None) or subgraph_isomorphic_completion_gt_in_generated
